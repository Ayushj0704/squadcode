  ...
  return (
    <div className="grid grid-cols-1 gap-6">
      <div className="rounded-2xl border-2 border-border bg-surface-0 shadow-card p-6">
        ...
        {error ? <ErrorState error={error} onRetry={() => void load().catch((e) => setError(errorMessage(e)))} /> : null}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        <div className="flex items-center gap-2 mb-2">
          <input
            type="text"
            placeholder="Search threads by title or contest..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full rounded-xl border-2 border-border bg-surface-0 px-4 py-2 font-bold text-ink-700 shadow-pop-sm focus:border-brand-500 focus:outline-none"
          />
        </div>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Create the first thread above to start discussing contests."
          ...
          threads
            .filter(t => 
              ...
              t.contestName.toLowerCase().includes(searchQuery.toLowerCase())
            )
            .map((t) => (
            <button
              key={t.id}
              onClick={() => navigate(`/threads/${t.id}`)}
              className="text-left rounded-2xl border-2 border-border bg-surface-0 shadow-card p-5 hover:bg-surface-raised/30 flex justify-between items-center"
            >
              <div>
                <div className="flex items-center gap-3">
                  {t.pinned ? (
                    <span className="flex items-center gap-1 rounded-lg bg-yellow-100 px-2 py-0.5 text-xs font-bold text-yellow-800">
                      📌 Pinned
                    </span>
                  ) : null}
              ...
              <div className="flex items-center gap-1 rounded-lg bg-surface-2 px-3 py-1 font-bold text-ink-600 border border-border">
                💬 {t._count?.posts ?? 0}
              </div>
            </button>
          ))
