...
/* ------------------------------------------------------------------ */
/*  LeetCode contest generation (deterministic – no API needed)        */
/* ------------------------------------------------------------------ */

const LC_WEEKLY_EPOCH   = new Date("2018-12-16T02:30:00Z").getTime();
const LC_BIWEEKLY_EPOCH = new Date("2020-01-25T14:30:00Z").getTime();
const WEEK_MS     = 7 * 24 * 60 * 60 * 1000;
const BIWEEK_MS   = 14 * 24 * 60 * 60 * 1000;
const LC_DURATION = 90 * 60 * 1000; // 1 h 30 min

function generateLeetCodeContests(count = 8): Contest[] {
  const now = Date.now();
  const contests: Contest[] = [];

  // Weekly: every Sunday 2:30 UTC
  const weeksSinceEpoch = Math.ceil((now - LC_WEEKLY_EPOCH) / WEEK_MS);
  for (let i = 0; i < count; i++) {
    const n = weeksSinceEpoch + i;
    const start = LC_WEEKLY_EPOCH + n * WEEK_MS;
    if (start > now - LC_DURATION) {
      contests.push({
        id: `lc-weekly-${n}`,
        platform: "leetcode",
        name: `Weekly Contest ${n}`,
        url: `https://leetcode.com/contest/weekly-contest-${n}`,
        startTime: start,
        durationMs: LC_DURATION,
      });
    }
  }

  // Biweekly: every other Saturday 14:30 UTC
  const biweeksSinceEpoch = Math.ceil((now - LC_BIWEEKLY_EPOCH) / BIWEEK_MS);
  for (let i = 0; i < count; i++) {
    const n = biweeksSinceEpoch + i;
    const start = LC_BIWEEKLY_EPOCH + n * BIWEEK_MS;
    if (start > now - LC_DURATION) {
      contests.push({
        id: `lc-biweekly-${n}`,
        platform: "leetcode",
        name: `Biweekly Contest ${n}`,
        url: `https://leetcode.com/contest/biweekly-contest-${n}`,
        startTime: start,
        durationMs: LC_DURATION,
      });
    }
...
async function fetchCodeforcesContests(): Promise<Contest[]> {
  ...
  if (!resp.ok) throw new Error("Codeforces API error");
  ...
  if (json.status !== "OK") throw new Error("Codeforces API returned non-OK");
  ...
  const loadContests = useCallback(async () => {
    ...
    try {
      const [cf, lc] = await Promise.all([
        fetchCodeforcesContests().catch(() => [] as Contest[]),
        Promise.resolve(generateLeetCodeContests(8)),
      ]);
      const all = [...cf, ...lc].sort((a, b) => a.startTime - b.startTime);
      setContests(all);
    } catch {
